...
def parse_args(input_args=None):
    ...
    parser.add_argument("--seg_ratio", 
                        default=0.08,
                        type=float, 
                        required=False,
                        help="The accepted mask area ratio")
...
def contour_wrapped(cnt_ref, cnt_qry):
    areas = [cv2.contourArea(cnt_ref), cv2.contourArea(cnt_qry)]
    outer = cnt_ref if areas[0] > areas[1] else cnt_qry
    inner = cnt_qry if areas[0] > areas[1] else cnt_ref
    # check if inner point is inside the outer contour
    #for point in inner:
        #pt = (point[0][0].astype(np.uint8), point[0][1].astype(np.uint8))
        #print(pt)
        #result = cv2.pointPolygonTest(outer, pt, False)
        #if result < 0:
        #    return False  # If any point is outside, return False
    inside = [cv2.pointPolygonTest(outer, tuple(point[0].astype(np.uint8)), False)<0 for point in inner] # inside<0 -> outside
    if inside.count(True) > 1e-1 * len(outer):
        return False # Points outside excesses borderline
    return True # Assuming all points are inside
...
def find_adjacent(current_contour, contours_set, allowed_distance=5):
    ...
    for query_idx, query_cnt in contours_set.items():
        ...
        #if count_shared_points > 100 or contour_intersect(current_contour, query_cnt):
        if count_shared_points > 50 or contour_wrapped(current_contour, query_cnt):
            adjacent_contours_set[query_idx] = query_cnt
    
    return adjacent_contours_set


def closest_points(query_array, ref_array, adjacent_bound=False):
    # iterate through each point in contour c1, c2
    min_dist = np.inf
    for point in query_array:
        diff = ref_array[:, 0] - point
        distance = np.einsum('ij,ij->i', diff, diff)
        point_ind = np.argmin(distance)
        if distance[point_ind] < min_dist:
            min_dist = distance[point_ind]
            ref_pt = ref_array[point_ind]
            qry_pt = point
    if adjacent_bound == False:
        return min_dist, (ref_pt, qry_pt)

    #print(f"min distance found {min_dist} {len(query_array)}, {len(ref_array)}")
    ref_bound = []
    qry_bound = []
    
    if min_dist > 1e2:
        print(cv2.contourArea(ref_array), cv2.contourArea(query_array))
        if cv2.contourArea(ref_array) > 10*cv2.contourArea(query_array):
            qry_bound = query_array
            #for q_id in range(len(query_array)):
            #    qry_bound.append(query_array[q_id])
        elif cv2.contourArea(query_array) > 10*cv2.contourArea(ref_array):
            ref_bound = ref_array
            #for r_id in range(len(ref_array)):
            #    ref_bound.append(ref_array[r_id])
        else:
            print(f"not as expected?")
        return ref_bound, qry_bound
    
    if min_dist > 10:
        return ref_bound, qry_bound

    # shared bound set (problematic)
    dist_range = min_dist * 4
    for point in query_array:
        diff = ref_array[:, 0] - point
        distance = np.einsum('ij,ij->i', diff, diff)
        point_ids = np.where(distance<=dist_range) 
        for point_ind in point_ids:
            #print("oob", point_ind, len(ref_array), ref_array[point_ind])
            if len(point_ind) > 0:
                for col in range(point_ind.shape[0]):   ref_bound.append(ref_array[point_ind[col]])
        if min(distance) <= dist_range:
            qry_bound.append(point)

    return qry_bound, ref_bound
...
def merge_masks(masks, im_w, im_h, accepted_ratio=0.08):
    # find contour center
    dist_list = []
    ...
    for init_idx in sort_idx[: init_range]:
        ...
        merge_area = np.count_nonzero(masks[init_idx]['segmentation']==True)
        merge_set = set()
        ...
        while merge_area < 0.1 * im_w * im_h and len(available_ids) > 0: # area \in (0.1, 0.4)
            ...
            merge_area += np.count_nonzero(masks[random_adj_idx]['segmentation'])
            count_boundaries = np.count_nonzero(touch_border(merge_seg_mask, border_type=['top', 'bottom', 'left', 'right'])==True, keepdims=False)
            if count_boundaries >= 2 or len(merge_set) > 5 or merge_area > 0.4 * im_w * im_h: # or len(merge_set) > 5 merge_area > 0.4 * im_w * im_h # len(merge_set) > 7  or merge_area > 0.6 * im_w * im_h
                break
            else:
                merge_set.add(random_adj_idx)
                available_ids = list(set(available_ids) - merge_set)
        
        # reject duplicate comb
        set_comb = dup_control(merge_set, set_comb)
    # TBC landmark
    mask_comb = []
    for exist_set in set_comb:
        comb_area = np.sum(list(map(lambda i: masks[i]['area'], exist_set)))
        print(f"merged area of set {exist_set} is {comb_area} {comb_area / (im_w*im_h)}")

        if comb_area > accepted_ratio * im_w * im_h:
...
def tight_mask_bound(contours):
    # find the shortest distance of any contour pair
    pair_cnts = list(combinations(np.arange(len(contours)), 2))
    pts_sets = [[] for _ in range(len(contours))]
    for pair_comb in pair_cnts:
        shared_bound0, shared_bound1 = closest_points(contours[pair_comb[0]], contours[pair_comb[1]], adjacent_bound=True)
        # collect pts in shared bounds
        if len(shared_bound0) != 0:
            for shared_pt in shared_bound0:
                pts_sets[pair_comb[0]].append(shared_pt)
        if len(shared_bound1) != 0:
            for shared_pt in shared_bound1:
                pts_sets[pair_comb[1]].append(shared_pt)

    
    # remove shared bounds
    pts_sets = [np.unique(np.vstack(pts), axis=0) if len(pts) != 0 else pts for pts in pts_sets]
    #print("roll back", pts_sets[0].shape, pts_sets[1].shape, contours[0].shape, contours[1].shape)
    
    for cnt_id in range(len(contours)):
        cnt_pts = contours[cnt_id].reshape(-1, 2)
        remove = np.isin(cnt_pts, pts_sets[cnt_id]).all(axis=1)
        #print(remove)
        contours[cnt_id] = cnt_pts[~remove].reshape(-1, 1, 2)
        #print(contours[cnt_id].shape)
    
    # sort pts in clockwise/couter-clockwise order
    contours = np.vstack(contours)
    M = cv2.moments(contours)
    centroid = (int(M["m10"] / M["m00"]), int(M["m01"] / M["m00"]))
    contours_sorted = sorted(contours, key=lambda point: angle_from_centroid(centroid, point[0]), reverse=False) # ccw ~ outer contour
    #contours_sorted = sorted(contours, key=angle_from_centroid, reverse=True) # cw ~ inner contour
    #print("stacked", len(contours))
    #print("sorted", len(contours_sorted))
    # smooth countour
    contours_sorted = np.array(contours_sorted, dtype=np.int32).reshape((-1, 1, 2))
    epsilon = 1e-4 * cv2.arcLength(contours, True)  # Adjust epsilon as needed
    approx_contour = cv2.approxPolyDP(contours_sorted, epsilon=0.1, closed=True)
...
def instance_outline_extraction_by_automask(mask_generator, input_dir, output_dir, device='cuda', img_format='png', image_resolution=1024, expand_pixel=3, seg_ratio=0.08):
    ...
    image_dim = 1024
    for batch_idx, (image, img_path) in enumerate(test_loader):
        #####################################
        if batch_idx <= 69041: # next run
            continue
        if batch_idx > 70000:
            break
        img_name = os.path.basename(img_path[0])
        #if '00a117153435875.632fcb179dd32' not in img_name:
        #if '004616116389897.6061453540f61' not in img_name:
        #if '00a117153435875.632fcb179dd32' not in img_name: # 0040e5173596733.6492ec9943529_0
        #    continue
        ...
        if len(masks) == 0:
            ...
            shutil.copy2(img_path[0], os.path.join(os.path.dirname(output_dir), 'removed', img_name))
            continue

        # HED extraction
        hed = HWC3(image_source) # np.array.shape (1024, 1024, 3 or 4)
        cv2.imwrite(os.path.dirname(output_dir) + '/hed_' + os.path.basename(output_dir) + '/' + img_id + '.' + img_format, hedDetector(hed))
        ...
        if output_dir is not None:
            ...
            for ann in sorted_masks:
                ...
                m = cv2.dilate(m.astype(np.uint8), kernel, iterations=3)
            ...
            try:
                filtered_mask_comb = merge_masks(sorted_masks, image_dim, image_dim, accepted_ratio=seg_ratio) # for now
                #sorted_masks = sorted(filtered_masks, key=(lambda x: x['area']), reverse=True)
                #print(f"mask comb returned {len(filtered_mask_comb)}")
                if len(filtered_mask_comb) == 0:
                    print(f"{img_id} has no masks that meet criterions.")
                    shutil.copy2(img_path[0], os.path.join(os.path.dirname(output_dir), 'removed', img_name))
                    continue
                ...
                for fid in range(len(filtered_mask_comb)):
                    ...
                    #print("counters", np.count_nonzero(mask_all==False), np.count_nonzero(mask_inn==False))

                    ###### fill holes inside product #######
                    mask_all = ~mask_inn # guess why
                    ...
                    ##### stack masks and extract tight outline #####
                    mask_contours, _ = cv2.findContours(1-mask_all.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
                    # remove random noisy contours
                    true_mask_contours = []
                    for msk_cnt in mask_contours:
                        msk_area = cv2.contourArea(msk_cnt)
                        if msk_area > 3600:
                            true_mask_contours.append(msk_cnt)

                    if len(true_mask_contours) > 1:
                        true_mask_contours = tight_mask_bound(true_mask_contours)
                        tabular = np.zeros((mask_all.shape[1], mask_all.shape[0]), dtype=np.uint8)
                        cv2.drawContours(tabular, [true_mask_contours], -1, 255, thickness=-1)
                        approx_contour, _ = cv2.findContours(tabular, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
                        true_mask_contours = approx_contour[0]
                    else:
                        true_mask_contours = true_mask_contours[0]
                    #print(f"tight segmentation contour: {true_mask_contours.shape}")
                    #true_mask_contours = [cv2.approxPolyDP(true_mask_contours, epsilon=0.01*cv2.arcLength, closed=False)]
                    # save contour
                    tabular = np.zeros((mask_all.shape[1], mask_all.shape[0]), dtype=np.uint8)
                    cv2.drawContours(tabular, [true_mask_contours], -1, 100, thickness=2)
                    #cv2.imwrite(os.path.join(output_dir.replace('sam2auto_seg', 'contour_sam2auto_seg'), img_id+f"_tight_{fid}.png"), tabular) # mask_all.astype(np.uint8)*255
                    ##### end of outline extraction #####
                    mask_tight = cv2.drawContours(tabular, [true_mask_contours], -1, args.hed_value, thickness=-1) # mask
                    mask_tight = cv2.resize(mask_tight, (OH, OW), interpolation=cv2.INTER_LINEAR)

                    # mask overlapping control
                    if len(regions_marked) == 0:
                        regions_marked.append(mask_all.astype(int))
                    else:
                        #print("final control", [np.sum(cv2.bitwise_xor(mask_all.astype(int), region)) for region in regions_marked], 1e-3*image_dim*image_dim)
                        if [np.sum(cv2.bitwise_xor(mask_all.astype(int), region))<1e-3*image_dim*image_dim for region in regions_marked].count(True):
                            #print(f"skip mask {fid}")
                            continue
                        else:
                            regions_marked.append(mask_all.astype(int))

                    #mask_inn = mask_tight #
                    mask_inn = cv2.dilate(mask_tight, kernel, iterations=3) # contour preserving
                    assert np.count_nonzero(mask_tight>0) > np.count_nonzero(mask_inn==True)
                    mask_inn = np.array(mask_inn, dtype=bool)
                    #mask_inn = ~mask_inn

                    mask_contour = mask_inn & ~mask_tight.astype(bool)
                    #print(f"count seg pixels {np.count_nonzero(mask_tight>0)} & {np.count_nonzero(mask_inn==True)} & {np.count_nonzero(mask_contour==True)}")
                    cv2.imwrite(os.path.join(output_dir.replace('sam2auto_seg', 'contour_sam2auto_seg'), img_id+f"_{fid}.png"), mask_contour.astype(int) * args.hed_value)


                    hed_dilated = hedDetector(hed) * ~mask_inn
                    hed_dilated = np.where(mask_contour==True, args.hed_value, hed_dilated)
                    hed_dilated = cv2.resize(hed_dilated, (OH, OW), interpolation=cv2.INTER_LINEAR)
                    img_masked = Image.fromarray(hed_dilated)   #Image.fromarray(hed)
                    img_save_path = os.path.dirname(output_dir) + '/mask_hed_' + os.path.basename(output_dir) + '/' + img_id + f'_{fid}' + '.' + img_format
                    img_masked.save(img_save_path, img_format)

                    hed_dilated = hedDetector(hed)
                    hed_dilated = np.where(mask_contour==True, args.hed_value, hed_dilated)
                    hed_dilated = cv2.resize(hed_dilated, (OH, OW), interpolation=cv2.INTER_LINEAR)
                    img_masked = Image.fromarray(hed_dilated)   #Image.fromarray(hed)
                    img_save_path = os.path.dirname(output_dir) + '/contour_hed_' + os.path.basename(output_dir) + '/' + img_id + f'_{fid}' + '.' + img_format
                    img_masked.save(img_save_path, img_format)


                    #hed_tight = hedDetector(hed) * ~mask_tight.astype(bool)
                    #hed_tight = np.where(tabular==True, args.hed_value, hed_tight)
                    #hed_tight = cv2.resize(hed_tight, (image_resolution, image_resolution), interpolation=cv2.INTER_LINEAR)
                    #img_masked = Image.fromarray(hed_tight)   #Image.fromarray(hed)
                    #img_save_path = os.path.dirname(output_dir) + '/mask_hed_' + os.path.basename(output_dir) + '/' + img_id + f'_tight_{fid}' + '.' + img_format
                    #img_masked.save(img_save_path, img_format)
...
if __name__ == "__main__":
    ...
    args = parse_args(['--input_dir', '/home/yangmi/Grounded-SAM-2/infer_dataset', 
                       '--output_dir', '/home/yangmi/Grounded-SAM-2/sam2auto_seg',
                       '--seg_ratio', '0.06'])
    device = torch.device(args.gpu_id)
    #device = torch.device(f"cuda:{args.gpu_id}")
    #print(f"Using device {device} {torch.cuda.device_count()}.")
    #assert int(args.gpu_id) < torch.cuda.device_count()
    ...
    os.makedirs(args.output_dir.replace('sam2auto_seg', 'contour_hed_sam2auto_seg'), exist_ok=True)
    os.makedirs(args.output_dir.replace('sam2auto_seg', 'mask_hed_sam2auto_seg'), exist_ok=True)
    os.makedirs(args.output_dir.replace('sam2auto_seg', 'removed'), exist_ok=True)
    '''
    # multiprocessing
    ...
    for rank in range(num_processes):
        ...
        p = mp.Process(target=instance_outline_extraction_by_automask, args=(mask_generator, args.input_dir, args.output_dir, device))
        p.start()
        processes.append(p)
    for p in processes:
        p.join() # # wait for all subprocesses to finish
    '''
    instance_outline_extraction_by_automask(mask_generator, input_dir=args.input_dir, output_dir=args.output_dir, device=device, seg_ratio=args.seg_ratio)